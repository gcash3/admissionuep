...
class ReportPage extends CSNCRUD  {
    ...
    function initialize() {
        ...
        $this->titles['T'] = array('table', 'College Freshmen Applicant');
        $this->titles['S'] = array('table', 'SHS - Grade 11 Applicants');  
        ...
        $columns0 = '';
        $columns1 = '[Semester][Campus;Campus][Course][AppliedThisDate;Applied This Date_@ASOF_][AppliedTotal;Applied Total][ApprovedThisDate;Approved This Date][ApprovedTotal;Approved Total][LastYearTotalApplied;Last Year Total Applied_@LASTASOF_][LastYearTotalApproved;Last Year Total Approved]';
        $columnsa = '';

        $this->columns = $columns1;

        // top button commands, modify parameters if necessar
        $this->addtopbuttoncommand("T", "Usp_AP_ADMS_GetApplied '$this->currentuserid', '$this->semester', '$this->campuscode', '[ABCDEFLN]', 'F', '[@searchtext]'", "$columns0 $columns1 $columnsa",'','',' ');        
        $this->addtopbuttoncommand("S", "Usp_AP_ADMS_GetApplied '$this->currentuserid', '$this->semester', '$this->campuscode', 'S', 'F', '[@searchtext]'", "$columns0 $columns1 $columnsa",'','',' ');                
    ...
    function callback_aftershowgrid(&$html, $results) {
        ...
        $table = '';
        $columns = $this->stringtoarray(true, $this->columns . "[LastYearFinalTotalApplied;(Final Data) Last Year Total Applied][LastYearFinalTotalApproved;(Final Data) Last Year Total Approved][LastYearFinalTotalEnrolled;(Final Data) Last Year Enrolled]");
        $table = HTML::datatable('Summary', $columns, $summary);
        $html .= HTML::box('Summary Report on Freshmen Applicants', $table, '', 'success');
