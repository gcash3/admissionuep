...
$(document).ready(function () { 
    ...
    if ($('.ApplicantData').length>0){
        $block = <?php echo $_SESSION["BlockCode"]; ?> + 0;
        if ($block > 0) {
        fBlockSchedule('<?php echo $_SESSION["BlockCode"]; ?>','< ? php echo Crypto::GenericChecksum($_SESSION[ "BlockCode"] );?>');
    }}

    //alert('< ?php echo $_SESSION["Date_Validated"]; ?>');
// ///
    // vBCode = < ?php echo $_SESSION["BlockCode"]; ?>;
    // if (vBCode > 0) {
    //     fBlockSchedule('< ?php echo $_SESSION["BlockCode"]; ?>','< ?php echo Crypto::GenericChecksum($_SESSION["BlockCode"]);?>');
...
//    $('#BLOCKSCHEDULE').append("<form method='post'> <div id = 'btnPrintBlock'> </br></br></br> <button type='submit' name='btnPrintBlock' class='btn btn-btn btn-success no-print'> <i class='fa fa-print' title=''> </i> Print </button> </div> <div id='btnGetPrintBlock'> </div></form>");
...
//$('#tbodySchedule').load("< ?php echo APP_BASE ?>fBlockSchedule/plain",{selectedCourseYear:'267640',cs:'267640'});
