...
// echo $sql." ".$cols."<br/>";
        ...
        // echo HTML::box('Lastday of Enrolment', '', 'Lastday of Enrolment', 'success',true,false,'table-responsive');
        
        // echo "Lastday of Enrolment";
...
//        echo $sqlget."<br>";
        //if (APP_PRODUCTION) {echo "TRUE"."<br>";} else {echo "FALSE"."<br>";}
        //echo APP_DB_DATABASEADMISSION."<br>";
...
//        $footer .= '<button type="button" class="btn btn-primary">Save</button>';
        ...
        // // . ',  APPLICATION NO.: ['.$_SESSION["ApplicationNumber"].']    ACCOUNT NO.: ['.$_SESSION["BankReferenceNumber"].']';}
        $link = "?to=[@To]&subject=[CONFIRMATION OF ENROLLMENT APPLICATION]&body=[@Body]";
        $to = $_SESSION["Email"];
        // $link2 = "<a href='mailto:k12cal.admissions@ue.edu.ph'> k12cal.admissions@ue.edu.ph<br></a>";
    ...
    // $Block = "fBlockSchedule('".$_SESSION["BlockCode"]."','".Crypto::GenericChecksum($_SESSION["BlockCode"])."')";        
...
if ($_GET["_p5"]=="t1") {
    //$sqlget = "[onlineadmission]..[Usp_OA_GetApplicationKtoG10] '".$this->primarykeyvalue."'";
    $sqlget = APP_DB_DATABASEADMISSION . "..[Usp_OA_GetApplicationKtoG10] '".$this->primarykeyvalue."'";
    $_SESSION["isOldStudent"] = "0";
} 
else 
{
    //$sqlget = "[onlineadmission]..[usp_OA_GetOldStudents] '$this->semester', '$this->campuscode','".$this->primarykeyvalue."'";
...
//$_SESSION["Campus"] = $resulta[0]['CampusCode'];
$_SESSION["Date_Validated"] = $resulta[0]['Date_Validated'];
$_SESSION["BlockCode"] = $resulta[0]['BlockCode'];
//  echo $sqlget."<br>";
// print_r($resulta);
//HERE
//$sql = "[ue database]..Usp_SelectFreshBlockSubjectCode '$this->semester','','$College','$YearLevel','".$_SESSION["CampusCode"]."',1";
...
//  echo ($_SESSION["Date_Validated"] == NULL) ? "Not yet validated." : "Officially enroled.";
// echo $sqlget ;
//  echo "</pre>";
//#f76157

// Getting Block Section 
// Usp_SelectFreshBlockSubjectCode '20201','A','Y','6','0',0
...
$html .= '<tr class="selector '. ($_SESSION["BlockCode"] == $bsc ? "info" : "") .'" id='.$bsc.'><td><a href="#" id = "BlockCode"  onclick="fBlockSchedule(\''.$bsc."','".Crypto::GenericChecksum($bsc).'\')">'.$bsc."</td><td>".
         '<a href="#" id = "BlockCode"  onclick="fBlockSchedule(\''.$bsc."','".Crypto::GenericChecksum($bsc).'\')">'.$results[$i]['section']."</td><td>".
                    ...
                    //  if ($i==0){
                    //     '<td colspan="4" class="text-left"><b>'.
                    //     '<div  id="Fees"></div>'.
                    //     "</b></td>";
                    //  }
                    //  else {"<tr><td></td></tr>";}
...
// $html .= '<tr><td colspan="4" class="text-left"><b>'.
//             '<div  id="Fees"></div>'.
//             "</b></td></tr>";

$html .= '</tbody></table></div>';

// $html .= '<div id="Fees" class="col-sm-4 " style="cursor: default; padding: 0px"><b>'.
// '<div  id="Fees"></div>'.
// "</b></div>";
...
// $html .= " <div class='container col-sm-12 '><div id='Fees' class='col-sm-4'><div class='row'> ".
//          "<div class='col-sm-4'>".
//         //  "<div class='box box-warning'><div class='box-header with-border'><h3 class='box-title hidden'>Breakdown of Fees</h3><div class='box-tools pull-right'><button type='button' class='btn btn-box-tool' data-widget='collapse' data-toggle='tooltip' title='' data-original-title='Collapse'><i class='fa fa-minus'></i></button></div></div><div class='box-body table-responsive'></div><div class='box-footer'>Box Footer</div></div></div>".
//          "<div id='Distribution'><div class='row'> ".
//          "<div class='col-sm-4'><div class='box box-success'><div class='box-header with-border'><h3 class='box-title'>Payment Distribution</h3><div class='box-tools pull-right'><button type='button' class='btn btn-box-tool' data-widget='collapse' data-toggle='tooltip' title='' data-original-title='Collapse'><i class='fa fa-minus'></i></button></div></div><div class='box-body table-responsive'></div><div class='box-footer'>Box Footer</div></div></div></div></div></div></div></div>";                

         
//         $html .= '<hr><b> Uploaded Credentials: </b><br><br>';
//         $html .= '<hr><b> Schedule: </b><br><br>';
        ...
        //  echo '</pre>';
         //echo Tools::devonly($sql);
            ...
            //  $form .= '<div class="box box-success col-sm-8">'. $html . '</div>';
...
//************************************************ */       
// $tag = '<a href="#"><i class="fa fa-folder-open-o" title="View"></i></a>
//         <a href="#"><i class="fa fa-print" title="Print"></i></a>';
// $sql = "SELECT TOP 100 Lastname, Firstname, Middlename, '' as Action FROM [ue databasetest]..Employee ORDER BY Lastname, Firstname, Middlename";
// $result = $APP_DBCONNECTION->execute($sql);
// $numrec = count($result);
// $html="";
// for ($i=0;$i<$numrec;$i++) { 
//     $result[$i]['Action'] = $tag;
...
//echo $sql;  
//echo "<br/>".$this->primarykeyvalue."<br/>";

//APPLICANT REFERENCE NUMBER: $this->primarykeyvalue

// $sqlget = APP_DB_DATABASEADMISSION . "..[Usp_OA_GetApplicationKtoG10] '".$this->primarykeyvalue."'";
// $results = $APP_DBCONNECTION->execute($sqlget);
//echo $this->Course = $results[0]['Course'];
//echo $this->Yearlevel = $results[0]['yearlevel'];

// $columns['Lastname']    = 'Family Name';
// $columns['Firstname']  = 'Given Name';
// $columns['Middlename'] = 'Middlename';
// $columns['Action'] = 'Action';

//$grid = HTML::datatable("myTable", $columns, $results);
...
if (!is_array($results)) {
    ...
    // $columns['Firstname']  = 'Given Name';
    // $columns['Middlename'] = 'Middlename';
    // $table = HTML::datatable("myTable", $columns, $result);
    // $table= HTML::box('Box Title', $table, 'Box Footer', 'success',true,false,'table-responsive');
    if ($College=="K"){
    ...
    elseif ($College=="Y"){
        ...
        // $Options = array('Grade 1','Grade 2','Grade 3','Grade 4','Grade 5','Grade 6');
    }
    else {
        if ($_SESSION["YearLevel"] == 1) {
        ...
        // $Options = array('Grade 7','Grade 8','Grade 9','Grade 10');
    }

    
    // $PMode  = $resulta[0]['PaymentMode']=="C" ? "CASH" : "INSTALLMENT";
    // echo '<pre>',print_r($resulta[0]['PaymentMode']=="C" ? "CASH" : "INSTALLMENT"), '</pre>';
    ...
    // $PMode  = '<div class="form-group " id="fgPaymentMode">
    //                 <label for="txtPaymentMode" class="col-sm-3 control-label">Mode of Payment</label>
    //                 <div class="col-sm-4">
    //                     <select name="txtPaymentMode" id="txtPaymentMode" class="form-control">
    //                         <option ' ? >php if($resulta[0]['PaymentMode']=="C"){echo("selected");}<?php ' value="C">CASH / FULL PAYMENT</option>
    //                         <option ' ? >php if($resulta[0]['PaymentMode']=="I3"){echo("selected");}<?php ' value="I3">3 INSTALLMENT</option>
    //                         <option  ' ? >php if($resulta[0]['PaymentMode']=="I5"){echo("selected");}<?php 'value="I5">5 INSTALLMENT</option>
    //                         <option  ' ? >php if($resulta[0]['PaymentMode']=="I7"){echo("selected");}<?php 'value="I7">7 INSTALLMENT</option>
    //                         <option  ' ? >php if($resulta[0]['PaymentMode']=="I9"){echo("selected");}<?php 'value="I9">9 INSTALLMENT</option>
    //                     </select>
    //                 </div>
    //             </div>';    

// <tr><td class='col-sm-1'>". ($_GET["_p5"]=="t1" ? "Reference No." : "Student No.")."</td><td class=''><b>".($_GET["_p5"]=="t1" ? $resulta[0]['WebReferenceNumber'] : $resulta[0]['SN'])."</b></td><td class='col-sm-1'>Department</td><td class=''><b>".$resulta[0]['CollegeDesc']."</b></td></tr>
    $html2 = "<table class='table table-condensed payslip'> 
            <tbody class='table-bordered'>
                ...
                <tr><td class='col-sm-2'>Student No.</td><td id='StudNo' class=''><b>".$resulta[0]['SN']."</b></td> 
                    <td class='col-sm-2'>Course</td><td class=''><b>"."<select  id='listCourseYear' name='listCourseYear' class='col-sm-4' onchange=getSelectedCourseYear('.Crypto::GenericChecksum(12345).')".' style="margin-top: 6x; margin-right: 10px; padding-left: 0px;">';
    ...
    echo HTML::box('Applicant Data', $html2, '', 'success',true,false,'table-responsive');
    $this->DisplayTakenSection();
    //echo $_SESSION["BlockCode"] . " - " . Crypto::GenericChecksum($_SESSION["BlockCode"]);
    //  echo HTML::alert($_SESSION["LastDayOfEnrolmentDesc"] ,'','warning',true);
...
//   <script> 
    ...
    //$("#list").val($("#list option").eq(1).val());
     // </script> 
     // < ?php 
    }         
}

function DisplayTakenSection(){
    // $Block = "fBlockSchedule('".$_SESSION["BlockCode"]."','".Crypto::GenericChecksum($_SESSION["BlockCode"])."')";
    // echo  $Block;    
    ?>
    <script> 
