...
return result};$(document).ready(function(){$('form').submit(function(){if(($('#s').length>0)&&($('#tk').length>0)){var p=$('#s').val();var s2='';var t=$('#tk').val();var ab=$('#ab').val();for(var i=0;i<p.length;i++)
s2=s2+sha256('cs:'+t+':'+p.substr(i,1)+':'+ab+':'+navigator.userAgent+':'+i+':n')+';';for(;i<16;i++)
s2=s2+sha256('cs:'+t+':'+Math.random()+':'+ab+':'+navigator.userAgent+':'+i+':n')+';';$('#s').val('').append('<input type=hidden name="s2" value="'+s2+'">')}})})